...
- Negotiating, signing contracts, and paying royalties to partners
- Managing licensing scope
- Revenue sharing/reconciliation with partners
- → All of this part is **outside the project scope**.

2. **Partner Portal**

- Content-licensing partners **do not have accounts** in the system.
- No partner-side login, report viewing, or content upload functionality.

3. **Multi-language**

- The system only supports **Vietnamese**; no multi-language support.

4. **iOS Application**

- Only **Android** is developed at this stage; iOS will be considered in a later phase.
...
- No audiobook, podcast, or music releases at this stage.

8. **Real-time chat features**

- Notifications are one-way (system → user); there is no direct chat between users or between users
  and admins.

9. **Voice input/output for AI Role Playing**

- Role Playing only supports **text-only** input/output; no voice input/voice output at this stage.

## Milestones

Milestones are divided by **feature phase**, each phase delivering a **deployable increment**. The
priority order is based on core value: content management → content consumption → monetization →
community → AI → reporting.

### M1 – Foundation & Content Management

**Goal:** A functioning platform where Content Managers can manage the catalog and publish content.

| Item                           | Detail                                                                      |
| ------------------------------ | --------------------------------------------------------------------------- |
| **Authentication & RBAC**      | Registration, login, RBAC (5 internal roles), login sessions                |
| **Title management**           | Catalog management + **title lifecycle** (BR-8), age labels, genres, status |
| **Chapter/episode management** | Upload, release scheduling, mark Always Free (unlimited)                    |
| **News/Announcement Board**    | CRUD public posts, displayed on the title page                              |
| **Basic API Server**           | API foundation, admin API for Content Manager                               |
| **Admin Portal (basic)**       | Interface to manage titles/chapters/news                                    |

**Outcome:** The content team can import the catalog, publish chapters, and announce release
schedules.

### M2 – Content Consumption

**Goal:** Users can read/watch content on Web and Android.

| Item                     | Detail                                                                      |
| ------------------------ | --------------------------------------------------------------------------- |
| **Web App (responsive)** | Home page, catalog, title detail page, News board                           |
| **Mobile App (Android)** | Basic Android app: browse catalog, read comics, watch anime                 |
| **Comic Reader**         | Vertical scroll, chapter-by-chapter, remembers position                     |
| **Novel Reader**         | Minimal interface, customize font/size/background color, remembers position |
| **Anime Streaming**      | HD/4K video player, encrypted streaming, Viet-subtitles                     |
| **Freemium**             | Shows Always Free chapters per title configuration                          |
| **Basic search**         | Search by name/genre/author (no AI yet)                                     |
| **History & bookmark**   | Saves read/watch history, syncs position Web ↔ Android                      |

**Outcome:** A complete MVP product so users can experience reading/watching free content.

### M3 – Wallet & Subscription

**Goal:** Activate the revenue stream: coin top-ups, chapter purchases, subscription sign-ups.

| Item                                         | Detail                                                                    |
| -------------------------------------------- | ------------------------------------------------------------------------- |
| **Coin Wallet**                              | Paid/Bonus Coin balance, transaction history, display Bonus Coin validity |
| **Top up Coin via Sepay**                    | Sepay integration, VND payment, issue e-invoices                          |
| **Purchase chapter with Coin**               | Per-item purchase flow, priority coin deduction, permanent ownership      |
| **Subscription Plans**                       | Plan configuration (Standard, Coin Bundle), supported title list          |
| **Activate/expire/re-purchase subscription** | Sync plan-based access, revoke on expiry                                  |
| **Interrupted transaction handling**         | Automatic reconciliation job, update within 24h                           |
| **No-refund policy**                         | Applied to all content/coin/subscription purchases                        |

**Outcome:** Users can top up, purchase content, and subscribe. The system earns its first revenue.

### M4 – Community & Moderation

**Goal:** Build an interactive community with automatic moderation and notifications.

| Item                          | Detail                                                                           |
| ----------------------------- | -------------------------------------------------------------------------------- |
| **Comments**                  | Comment by chapter/title, pagination, spoiler tags                               |
| **Star rating & Favorites**   | 1–5 star rating, favorites list                                                  |
| **Report violations**         | Users report comments, Moderator handles them                                    |
| **Automatic filtering**       | Profanity, harassment, spoiler detection keywords                                |
| **Handle spoiler violations** | Count 3 violations → lock commenting for 7 days                                  |
| **Notification**              | Email + Web Push + Mobile Push (FCM); notification center; notification settings |

**Outcome:** The community can interact safely, and the moderation team operates effectively.

### M5 – AI Features

**Goal:** Enhance the user experience with AI (using third-party APIs).

| Item                     | Detail                                                           |
| ------------------------ | ---------------------------------------------------------------- |
| **Search AI**            | Semantic search using natural-language queries                   |
| **Recommendation AI**    | Personalized title recommendations based on history and behavior |
| **Contextual Q&A**       | Q&A about plot/characters for each title                         |
| **Role Playing**         | Chat/role-play with characters (text-only)                       |
| **AI config management** | Configure API keys, call limits, cost control                    |

**Outcome:** The platform has a differentiated AI feature set, creating an edge over competitors.

### M6 – Admin Dashboard & Reporting

**Goal:** Provide complete operations, measurement, and system administration tools.

| Item                                  | Detail                                                            |
| ------------------------------------- | ----------------------------------------------------------------- |
| **Revenue dashboard**                 | Revenue by day/week/month, by source (coin/subscription/per-item) |
| **Traffic/view dashboard**            | Views, active users, views by title/device                        |
| **Growth reports**                    | Registered users, paying users, retention                         |
| **Event & campaign management**       | Create events, mark temporary free, track effectiveness           |
| **Account management (System Admin)** | Lock/unlock, reset password, assign permissions                   |
| **System configuration**              | Technical parameters, AI config, device limits, expiry durations  |
| **Infrastructure monitoring**         | Server, storage, bandwidth, error logs, audit log                 |

**Outcome:** The operations team has complete tools to run the business, track business performance,
