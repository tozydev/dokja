...
status code. `type` is not emitted; `instance` is the request path. Trace correlation travels in the
`X-Trace-Id` response header, set on error responses only (see [§10](#10-observability)).
...
```json
{
  ...
  "instance": "/api/v1/wallet/transactions",
...
| `detail`    | no       | Detailed, human-readable explanation                                                                                     |
| `instance`  | no       | URI of the specific request occurrence (the request path)                                                                |
| `timestamp` | yes      | Extension member: ISO-8601 UTC timestamp of the failure                                                                  |
| `code`      | no       | Extension member: stable, machine-readable error code; present on **domain errors only**                                 |
| `errors`    | no       | Extension member: validation violations; `field` (body) or `parameter` (query/handler) + `message`; never carries `code` |

Error codes are **lower snake_case** (`{domain}_{reason}`), e.g. `wallet_insufficient_balance`,
`anime_state_conflict`, `age_restricted`. They are stable: once published, a code is never renamed
or removed without a major version bump. Infrastructure errors — validation, authentication, 5xx —
never carry `code`.

How to produce errors in code: see [backend error-handling guide](backend/01-error-handling.md).
